...
def get_optim(model, config):
    ...
    result = {"optim": getattr(torch.optim, config["train_params"]["name"])(
...
def optim_step(model, opt_dict, loss, config):
    ...
    if not _valid_gradient(model):
        raise Exception("Issues with gradient")
        opt_dict['optim'].zero_grad() # Zero out the gradient if the gradients contain nan or inf

    else:
        if config["splitter"]["clip_grad_norm"] > 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), config["splitter"]["clip_grad_norm"])
