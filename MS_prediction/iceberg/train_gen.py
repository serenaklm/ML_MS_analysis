...
from model.gen_model import FragGNN
...
def get_dataloaders(config):
    ...
    magma_folder = config["dataset"]["magma_folder"]
    num_workers = config["process_setting"].get("num_workers", 0)
    magma_tree_folder = data_dir / dataset_name / f"{magma_folder}/magma_tree"
...
def train(config):
    ...
    # Create model
    model = FragGNN(**config["model"]["params"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", type = str, default = "gen_config.yaml", help = "Config file")
