...
from ms_pred.massformer_pred import massformer_data

from model.massformer_model import MassFormer
...
def get_dataloaders(config):
    ...
    form_dir_name = data_dir / dataset_name / Path(config["dataset"]["form_dir_name"])
...
def train(config):
    ...
    results_dir = os.path.join(config["args"]["results_dir"], "massformer")
    ...
    # Get the model 
    model = MassFormer(**config["model"]["params"])
