...
from utils import read_config, load_pickle, pickle_data, load_json
...
def get_datasets(params, split_filepath):
    ...
    split = load_json(split_filepath)

    train_ids = [os.path.join(params["data"]["dir"], f) for f in split["train"]] 
    val_ids = [os.path.join(params["data"]["dir"], f) for f in split["val"]] 
    test_ids = [os.path.join(params["data"]["dir"], f) for f in split["test"]] 

    train_data = Data(train_ids, dataset.process)
    val_data = Data(val_ids, dataset.process)
    test_data = Data(test_ids, dataset.process)

    # Look at 2 sets - val and test
    print(f"Analyzing : {len(val_data)} val samples and {len(test_data)} test samples")
    
    return train_data, val_data, test_data, split["train"], split["val"], split["test"]

def get_modules(model_cache_folder, params, split_filepath, include_adduct, include_CE, include_instrument):
    ...
    train_data, val_data, test_data, train_ids, val_ids, test_ids = get_datasets(params, split_filepath)

    return model, task, train_data, val_data, test_data, train_ids, val_ids, test_ids

def get_influence_scores(folder, output_path_val, output_path_test, split_filepath):
    ...
    # Get the model 
    model, task, train_data, val_data, test_data, train_ids, val_ids, test_ids = get_modules(folder, params, split_filepath,
    ...
    analyzer.compute_pairwise_scores(
        scores_name="val_scores",
        factors_name="EKFAC",
        query_dataset=val_data,
    ...
    # Load the scores 
    val_scores = analyzer.load_pairwise_scores(scores_name = "val_scores")
    test_scores = analyzer.load_pairwise_scores(scores_name = "test_scores")
    
    # Save the scores
    pickle_data(val_scores, output_path_val)
    pickle_data(test_scores, output_path_test)

    # Save the ids
    pickle_data(train_ids, folder / "train_ids.pkl")
    pickle_data(val_ids, folder / "val_ids.pkl")
...
if __name__ == "__main__":
    ...
    # Get the considered dataset 
    datasets = ["massspecgym_sieved", "nist2023_sieved"]

    # Get the list of checkpoints that we want to obtain influence scores for 
    all_checkpoints = [] 

    for dataset in datasets:
        dataset_folder = os.path.join(folder, dataset)
        checkpoints = [f for f in os.listdir(dataset_folder) if "random" in f]
        for c in checkpoints: 
            all_checkpoints.append(os.path.join(dataset_folder, c))
    ...
    for f in all_checkpoints:
        ...
        f = Path(f)
        output_path_val = f / "EK-FAC_scores_for_sampling_val.pkl"
        output_path_test = f / "EK-FAC_scores_for_sampling_test.pkl"
        if os.path.exists(output_path_val) and os.path.exists(output_path_test): 
            print(f"{output_path_val} already exists. Continue.")
            continue

        print(f"Getting the influence scores for: {f}")

        # Get the split file 
        split_filepath = os.path.join(splits_folder, dataset, "splits", "sampling_split.json")
        get_influence_scores(f, output_path_val, output_path_test, split_filepath)