...
@torch.no_grad()
def forward(model_name, model, batch, device):

    if model_name == "binned_MS_encoder":
        
        FP_pred, _ = model(batch["binned_MS"].to(device))
        ...
        FP_pred, _ = model(mz, intensities, mask, binned_ms)
        ...
        FP_pred, _ = model(intensities, formula, mask, binned_ms)
...
def predict(model, config, device, batch_size, threshold = 0.5):
    ...
    for batch in tqdm(data_loader):
        ...
        # Forward pass
        FP_pred = forward(model_name, model, batch, device)
        FP_pred = FP_pred.cpu()
